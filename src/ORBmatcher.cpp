...
int ORBmatcher::SearchByProjection(Frame &F, const vector<MapPoint *> &vpMapPoints, const float th)
{
    ...
    for (size_t iMP = 0; iMP < vpMapPoints.size(); iMP++) //遍历所有MapPoints
    {
        ...
        // 通过投影点搜索窗口和预测尺度进行搜索，找出附近兴趣点
        const vector<size_t> vIndices =
            F.GetFeaturesInArea(pMP->mTrackProjX, pMP->mTrackProjY, r * F.mvScaleFactors[nPredictedLevel], nPredictedLevel - 1, nPredictedLevel);

        if (vIndices.empty()) //没有找到兴趣点
            continue;
                                       ...
                                       vector<pair<size_t, size_t>> &vMatchedPairs, const bool bOnlyStereo)
{
    ...
    // 步骤0：得到KF1的相机光心在KF2中的坐标（KF1在KF2中的极点坐标），即投影
...
int ORBmatcher::Fuse(KeyFrame *pKF, const vector<MapPoint *> &vpMapPoints, const float th)
{
    ...
    for (int i = 0; i < nMPs; i++)
    {
        ...
        cv::Mat p3Dc = Rcw * p3Dw + tcw;   // 求MP在相机坐标系下的坐标
        ...
        // Point must be inside the image
        if (!pKF->IsInImage(u, v)) // 如果Point不在图片中
            continue;

        const float ur = u - bf * invz; // z=fb/d, d=ul-ur, ul即u
        ...
        const vector<size_t> vIndices =
            pKF->GetFeaturesInArea(u, v, radius);
        ...
        if (bestDist <= TH_LOW) // 找到了MapPoint在该区域最佳匹配的特征点
        {
            ...
            else // 如果这个点没有对应的MapPoint，则添加该点的MapPoint
            {
...
// 在上一帧的MapPoint投影到当前帧，通过速度模型可估计当前帧Tcw
// 在投影点附近根据描述子距离选取匹配，以及最终方向投票机制来剔除
